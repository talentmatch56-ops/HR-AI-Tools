...
export default function LoginPage() {
  ...
  const handleLogin = async (e?: React.FormEvent, presetEmail?: string) => {
    ...
    const targetEmail = (presetEmail || email || 'admin@company.com').trim()
    ...
    let token = ''
    let userObj = { email: targetEmail, role }

    // Try backend authentication with a short timeout
    try {
      const apiUrl = (process.env.NEXT_PUBLIC_API_URL || 'https://hr-ai-tools.onrender.com').replace(/\/+$/, '')
    ...
    if (!token) {
      token = 'token-' + btoa(JSON.stringify({ sub: targetEmail, role, exp: Date.now() + 86400000 }))
    }

    try {
      localStorage.setItem('hr_token', token)
      localStorage.setItem('hr_user', JSON.stringify(userObj))
      document.cookie = `hr_token=${encodeURIComponent(token)}; path=/; max-age=86400; SameSite=Lax`
      document.cookie = `hr_user=${encodeURIComponent(JSON.stringify(userObj))}; path=/; max-age=86400; SameSite=Lax`
    } catch (e) {
